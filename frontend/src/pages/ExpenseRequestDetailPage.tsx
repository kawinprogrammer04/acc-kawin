...
export function ExpenseRequestDetailPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-5 p-6">
    ...
    <Card><CardContent className="p-0">
      ...
      {pendingStep && <div className="border-t px-6 pb-6 pt-5">
        <button onClick={approve} disabled={saving || (!signature && !useSavedSignature)}
          className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground disabled:opacity-100">
          <CheckCircle2 className="h-4 w-4" /> ยืนยันอนุมัติและประทับลายเซ็น
        </button>
      </div>}
    </CardContent></Card>
